...
* **整段一次生成**：不按句拆开分别合成再拼接 —— 那样每段是独立随机采样，接缝处音色会变
...
ZipVoice 有两个精度的模型，`download_models.py` 默认装 int8，加 `--fp32` 再装一份 fp32：

| | int8（默认） | fp32 |
| --- | --- | --- |
| 磁盘 | 146 MB | 494 MB |
| RTF（8 核 arm64） | ~0.9 | ~2.2 |
| 46 字首段音频 | ~2.3 s | ~9.7 s |

**作者没能客观测出两者音质差异。** 一开始单次测量显示 fp32 的高频噪声只有 int8 的 1/3，
但重复 3 次后 fp32 变成 `0.0097 ± 0.0075`（int8 是 `0.0097`）——标准差比两者差值还大，
那个"3 倍"是抽样运气。频谱平坦度同样分不开。

所以默认取快的 int8。想自己听：

```bash
ZIPVOICE_MODEL=sherpa-onnx-zipvoice-distill-fp32-zh-en-emilia bash start.sh
```

**顺带一个反直觉结论**：`num_steps` 调大**不会**更好。实测 4→16→32 步，
频谱平坦度 0.06→0.12→0.15、高频能量 0.006→0.023，噪声翻倍。默认 4 步就是对的。
...
如果你自己写脚本调这个服务，**不要**把长文本切成一句一句分别 `generate` 再拼接。
每次 `generate` 都是独立随机采样，段与段之间音色会漂移——听感就是
"前半句一个声音，后半句换成另一个声音"。

服务内部因此是**整段一次生成**；流式模式下也只在超过 60 字时才分段。
